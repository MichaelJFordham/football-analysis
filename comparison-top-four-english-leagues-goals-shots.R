...
champ_2023_shooting <- fb_season_team_stats(country = "ENG", gender = "M", season_end_year = "2023", tier = "2nd", stat_type = "shooting")
champ_2023_shooting <- champ_2023_shooting %>%
  filter(Team_or_Opponent == "team") %>%
  select(Squad, Gls_Standard, Sh_Standard)

lg1_2023_shooting <- fb_season_team_stats(country = "ENG", gender = "M", season_end_year = "2023", tier = "3rd", stat_type = "shooting")
lg1_2023_shooting <- lg1_2023_shooting %>% 
  filter(Team_or_Opponent == "team") %>%
  select(Squad, Gls_Standard, Sh_Standard)

lg2_2023_shooting <- fb_season_team_stats(country = "ENG", gender = "M", season_end_year = "2023", tier = "4th", stat_type = "shooting")
lg2_2023_shooting <- lg2_2023_shooting %>% 
  filter(Team_or_Opponent == "team") %>%
  select(Squad, Gls_Standard, Sh_Standard)

shooting <- do.call("rbind", list(prem_2023_shooting, champ_2023_shooting, lg1_2023_shooting, lg2_2023_shooting))
